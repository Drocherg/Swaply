<template>
      ...
      <div v-if="showLogoutModal" class="modal-overlay" @click="cancelLogout">
        <div class="modal-content" @click.stop>
          ...
          <div class="modal-buttons">
            <ion-button fill="clear" class="cancel-button" @click="cancelLogout">
              No
            </ion-button>
            <ion-button class="confirm-button" @click="confirmLogout">
              Sí
            </ion-button>
          </div>
        </div>
    ...
    const helpMessages = ref([
      { type: 'received', text: '¡Hola! ¿En qué puedo ayudarte con los ajustes?' }
    ]);
    ...
    const sendHelpMessage = () => {
      ...
      setTimeout(() => {
        ...
        if (lowerCaseMessage.includes('datos')) {
          response = 'En "Datos personales" puedes modificar tu información de perfil.';
        } else if (lowerCaseMessage.includes('notificacion')) {
          response = 'En "Notificaciones" puedes configurar qué alertas quieres recibir.';
        } else if (lowerCaseMessage.includes('sesion') || lowerCaseMessage.includes('cerrar')) {
          response = 'Para cerrar sesión, pulsa el botón rojo y confirma tu decisión.';
        }
    ...
    const goToPersonalData = () => router.push('/Datos1');
    const goToNotifications = () => router.push('/AjustesNotis1');
